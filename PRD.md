...
**Описание.** Визуальный концепт: ржаво-красная палитра, тёмный фон. Марсианское настроение создаётся фоновым свечением (фиксированный слой с радиальными оранжево-ржавыми градиентами) и небольшим числом лёгких оранжевых частиц на фоне секции Hero. Отдельный компонент фона и изображение Марса не используются.
...
**Technical considerations:**
- Реализация фона — только CSS: фиксированный слой свечения `.bg-glow` (`z-index: -1`, `overflow: hidden`, `pointer-events: none`; радиальные градиенты `rgba(214,110,54,0.24)` и `rgba(150,60,30,0.20)`) + слой частиц внутри `HeroSection` (`position: absolute`, `pointer-events: none`, `aria-hidden`). Без внешних изображений и тяжёлых библиотек.
- Частицы: немного (6–10), оранжевого цвета, без сильного свечения; движение отключается при `prefers-reduced-motion`.
- Контраст текста к фону на пике свечения остаётся ≥ 4.5:1 (учтено цветом `--color-text-dim`).
...
[
  {
    ...
    "description": ["string", "..."],  // Описание группы — массив абзацев (как details в experience.json)
    "items": ["string", "..."]         // Список навыков/технологий в группе
  }
]
```
- `SkillGroup.Description` — список абзацев (`List<string>`); каждый элемент рендерится отдельным `<p>` в раскрытой карточке группы (аналогично `ExperienceItem.Details`). Пустой/отсутствующий массив допустим.
...
| — | Фоновое свечение `.bg-glow` (`index.html`) и динамические частицы Hero (разметка внутри `HeroSection.razor`) |
