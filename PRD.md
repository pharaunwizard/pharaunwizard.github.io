...
**Описание.** Визуальный концепт: ржаво-красная палитра, тёмный фон. Марсианское настроение создаётся фоном страницы (радиальные градиенты на `body`) и небольшим числом лёгких оранжевых частиц на фоне секции Hero (без отдельного фонового компонента, изображений, свечений и параллакса).
...
**Technical considerations:**
- Реализация фона — только CSS: радиальные градиенты на `body` + слой частиц внутри `HeroSection` (`position: absolute`, `pointer-events: none`, `aria-hidden`), без внешних изображений и тяжёлых библиотек.
...
| — | Частицы фона Hero — разметка внутри `HeroSection.razor` (без отдельного компонента) |
